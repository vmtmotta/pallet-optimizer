...
// 1) Load master data (array of {REF, PRODUCT, Box1 Units, Box1 Dimensions…, Box2 Units, Box2 Dimensions…})
window.addEventListener('DOMContentLoaded', async () => {
  try {
    let r = await fetch('products-detail.json');
    let arr = await r.json();       // MUST be a top-level array
    arr.forEach(p=>{
      productsBySku[p.REF] = {
        name:       p.PRODUCT,
        // Box1
  ...
  return new Promise((res,rej)=>{
    ...
    fr.onload = e => {
      try {
        ...
        let hdr = rows.find(r=> r.includes("REF") && r.includes("BOX USED (BOX1 or BOX2)") && r.includes("ORDER IN UNITS"));
...
// 3) compute best same-SKU count (5+2)
function bestSingleGridCount(d, canRotate){
  ...
  opts.forEach((o1,i1)=>{
    let rows=Math.floor(L/o1.l), cols=Math.floor(W/o1.w), base=rows*cols;
    let remL=L-rows*o1.l, remW=W-cols*o1.w, extra=0;
    opts.forEach((o2,i2)=>{
      if(i1===i2) return;
...
function packLayer(insts){
  ...
  if(insts.every(x=>x.sku===sku0)){
    let pd=productsBySku[sku0],
        ...
        max=bestSingleGridCount(dims,canR),
        take=Math.min(max, insts.length);
    return { placed:insts.slice(0,take).map(b=>({box:b})), notPlaced:insts.slice(take) };
  }
...
async function optimize(){
  ...
  catch(e){ return alert("Error reading file: "+e.message); }
  if(!lines.length){
    document.getElementById("results").innerHTML="<p><em>No valid lines</em></p>";
    return;
  }
  // expand into box instances
  let insts=[];
  lines.forEach(l=>{
