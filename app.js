...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    console.log('productsBySku ready:', productsBySku);
  } catch (err) {
    console.error('Error loading master data:', err);
    alert('Could not load product master data; packing may fail.');
  }
});

// 2) Read the Excel order file
...
// 3) Compute maximum same‐SKU grid count per layer
...
// 4) Pack one layer (single‐SKU grid or mixed‐SKU guillotine)
...
async function optimize() {
  ...
  if (!fileEl.files.length) return alert('Select an order file');
  ...
  if (!lines.length) {
    return document.getElementById('results')
      .innerHTML = '<p><em>No valid order lines.</em></p>';
  }
  ...
  if (!insts.length) {
    return document.getElementById('results')
      .innerHTML = '<p><em>No boxes after expansion.</em></p>';
  }

  // pack pallets
  let rem = [...insts], pallets=[];
  while (rem.length) {
    let usedH=0, palletWT=PALLET_EMPTY_WT;
    const layers=[];
    while (true) {
      ...
      usedH += layerH;
      palletWT += placed.reduce((s,x)=>s+x.box.weight,0);
      layers.push(placed);
      rem = notPlaced;
    }
    pallets.push({layers, height:usedH, weight:palletWT});
  }
